...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  try {
    ...
    // Upsert profile — creates it if missing (handles cases where trigger failed)
    ...
    const displayName = freshProfile?.full_name?.split(' ')[0] ?? user.email?.split('@')[0] ?? 'Account'
    const credits = freshProfile?.letter_credits ?? 0
    ...
    // If anything crashes, redirect to login
    redirect('/auth/login')
  }
}

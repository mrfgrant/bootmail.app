      ...
      <div className="space-y-0.5">
        {waitlist.map(entry => (
          <div key={entry.id}
            ...
            <div>
              <div style={{ fontFamily: 'var(--font-body)', fontSize: '14px', color: '#ffffff' }}>{entry.name ?? <span style={{ color: '#4a4a40' }}>No name</span>}</div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: '#4a5240', marginTop: '2px' }}>{entry.email}</div>
            </div>
            <div className="text-right">
              {entry.branch && <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: '#d4a017' }} className="uppercase">{entry.branch}</div>}
